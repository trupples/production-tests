...
#unzip -o $FW_DOWNLOAD_PATH/ad-acevsecrdset-sl.zip -d $FW_DOWNLOAD_PATH
#ret=$?

ret=1 # Use local firmware image (for now!)

if [ $ret == 0 ];then
    echo "wget success"
    daplink_upload $FW_DOWNLOAD_PATH/ad-acevsecrdset-sl.hex
else
    echo "wget error"
